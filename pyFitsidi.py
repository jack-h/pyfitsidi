...
def parseConfig(tagname, config='config.xml'):
    """
    A wrapper to choose a parser based on config file
    extension.

    Currently supports:
    .xml files -> calls parseConfigXml(tagname, config)
    .yaml files -> calls parseConfigYaml(tagname, config)
    """
    file_ext = config.split('.')[-1]
    if file_ext == 'yaml':
        return parseConfigYaml(tagname, config=config)
    else:
        # default to Xml case, which can act on tree onjects as well as filenames
        return parseConfigXml(tagname, config=config)

def parseConfigYaml(tagname, config='config.yaml'):
    """ Finds tagname in configuration file configs and returns dictionary of values
  This is a helper function, and is not usually called directly.
  """

    import yaml
    with open(config, 'r') as fh:
        c = yaml.load(fh.read())['config']
    return c[tagname]


def parseConfigXml(tagname, config='config.xml'):
...
def make_array_geometry(config='config.xml', num_rows=1):
  ...
  * DIAMETER: Antenna diameter (optional)
  ...
  """
    ...
    tblhdu = pf.BinTableHDU.from_columns(c, nrows=num_rows)
