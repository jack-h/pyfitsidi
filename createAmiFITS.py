...
def main():
  ...
  p.add_option('--h5file', dest='h5file', type='string', default='',
      help='Input HDF5 data file to convert.')
  p.add_option('--fitsfile', dest='fitsfile', type='string', default='',
      help='Output fits file. If none is given, the input file will be used with a .fits extension')
  p.add_option('--configfile', dest='configfile', type='string', default='',
      help='Config file for setting up FITS headers')
  ...
  if not os.path.isfile(hdffile):
    raise RuntimeError('Input hdf5 file: "%s" does not exist or is not a file!'%hdffile)

  if opts.fitsfile == '':
    fitsfile = hdffile + '.fits'
    fitsfile_dir = os.path.dirname(fitsfile)
  if not(os.path.isdir(fitsfile_dir)):
    raise RuntimeError('Output directory: "%s" does not exist'%fitsfile_dir)

  configfile = opts.configfile
  if not os.path.isfile(configfile):
    raise RuntimeError('Input configfile: "%s" does not exist or is not a file!'%configfile)
  ...
  print('------------------------------------')
  hdu = make_primary(config=configfile)
